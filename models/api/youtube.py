...
class Youtube:
    ...
    @staticmethod
    def get_channel_statistics(channel_id, *args):#channel_views=False, channel_videos=False, channel_subscriptions=False,
        ...
        #              and i != 'country']
        parameter = [i for i in args[0] if i != 'country']
        data = {}
        channel = yt.get_channel_metadata(channel_id)
        if parameter:
            for i in parameter:
                data[i] = channel[i.split('_')[-1][:-1]+'_count']
        if 'channel_country' in args[0]:
            data['channel_country'] = channel['country']
        return data

    @staticmethod
    def get_count_videos_playlists_of_channel(channel_id, *args):#max_playlists_videos=False, min_playlists_videos=False):
        data = {}
        current_playlists = [i['playlist_n_videos'] for i in yt.get_playlists(channel_id)]
        if 'max_playlists_videos' in args[0]:
            data['max_playlists_videos'] = max(current_playlists)
        if 'min_playlists_videos' in args[0]:
    ...
    @staticmethod
    def get_video_data(channel_id, *args):#max_video_created_at=False, max_video_likes=False, max_video_comments=False,
        ...
        # frame = inspect.currentframe()
        account_parameter = [i for i in args[0] if 'account' in i and 'created_at' not in i]
        max_parameter = [i for i in args[0] if 'max' in i and 'created_at' not in i]
        min_parameter = [i for i in args[0] if 'min' in i and 'created_at' not in i]
